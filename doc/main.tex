...
\begin{table}[h]
\parbox{.45\linewidth}{
  \caption{Plant based crafted diet}
\centering
\begin{tabular}{c c}
\toprule
Food Category & Grams\\
...
\paragraph*{Emissions calculation} The total emissions of a daily diet has been computed by the sum of the single contributes of every food category in kgCO2eq per kg, each calculated in proportion of the amount consumed in the diet as in \Cref{eq:diet}.

\begin{equation}\label{eq:diet}
  emissions_{food} = \frac{\textrm{Total Emissions of the food category} * \textrm{Grams contained in the diet}}{1000}
\end{equation}
...
We found based on statistical analysis that not only total emissions but also GHG accounting for nutritional value (in kgCO2eq per 1000kcal) have a p-value of  $p= 2.2\cdot10^{-6} $  and $p= 1.4\cdot10^{-5}$   for the latter one when conducting the unequal variance test with $\alpha = 0.01$. With regards to the diet examples comparison, as seen in \Cref{tbl:results-diets} the plant-based diet emits $2.02$ kgCO2eq while the omnivor one $5.42$ kgCO2eq, i.e. $268\%$ of the first one.
...
Furthermore, it is arguably up for discussion as to how different greenhouse gases are weighted  when calculating the ... as described in  \Cref{data}. For instance, methane is a much shorter-loved but more powerful greenhouse gas then 

...
\begin{abstract}
  Food is responsible for one quarter of the world's greenhouse gas (GHG) emissions \cite{Poore2018}. One of the greatest challenges is that everybody has access to a nutritious diet in a sustainable way. In this study, we investigate if plant-based diets do emit significantly less GHG than animal based-products. We found that not only total emissions but also GHG accounting for nutritional value (in kgCO2eq per 1000kcal) have a p-value of 0.00022\% and 0.0014\% for the latter one when conducting the unequal variance test with $\alpha = 0.01$. Our results demonstrate a significant difference between plant- and animal-based products, not only w.r.t. their total emissions but also when accounting for nutritional value. For people wanting to  modify their lifestyle towards a more sustainable one, an easy rule to guide their daily food-decisions derived from this study would be: Eat plant-based!
\end{abstract}
